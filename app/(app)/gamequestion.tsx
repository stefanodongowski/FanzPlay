...
// create a component
const GameQuestionPage = ({ gameID }: { gameID: string }, { team}: {team: Team}) => {
  ...
  const handleSubmit = () => {
    ...
    if (game.gameState === 'question' && selectedAnswer === currentQuestion.correctAnswer){
    ...
    <View style={[styles.container, styles.dark]}>
      ...
      {currentQuestion.answers.map((answer, index) => (
        <Pressable
          key = {index}
          onPress = {() => handleAnswerSelect(index)}
          style = {({ pressed }) => [styles.answerButton, 
